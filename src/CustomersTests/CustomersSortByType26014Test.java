...
public class CustomersSortByType26014Test {
    ...
    @Test
    public void test26014CustomersSortByType() throws Exception {
        ...
        WebElement element = driver.findElement(By.xpath("//div[5]/div/div/div[2]"));
        Actions actions = new Actions(driver);
        actions.moveToElement(element).click().build().perform();
        String resultSortedTable = js.executeScript("let tabl = document.getElementsByClassName('tabulator-cells'); let columnValues = [];let a = document.getElementsByClassName('tabulator-cell');for(let i = 4; i < a.length; i+= 9){columnValues.push(a[i].title);}for(let i = 0; i <columnValues.length - 1; i++){if(tabl[i] <= tabl[i + 1]){return false;}return true;}").toString();
        try {
        ...
        element = driver.findElement(By.xpath("//div[5]/div/div/div[2]"));
