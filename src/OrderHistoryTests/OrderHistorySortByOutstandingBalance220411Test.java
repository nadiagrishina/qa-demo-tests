...
public class OrderHistorySortByOutstandingBalance220411Test {
    ...
    @Test
    public void test220411OrderHistoryASCDESC() throws Exception {
        ...
        js.executeScript("document.querySelector('.tabulator-col:nth-child(9) .tabulator-col-sorter').click();");
        String resultSortedTable = js.executeScript("tabl = document.getElementsByClassName('tabulator-cells');filterField = document.getElementsByClassName('balance').value;let l = []; let w = 0;for (let i = 8; i<tabl.length;i = i + 15){l[w] = tabl[i]; w++;}for(let i =0; i< l.length; i++){let a = l[i].firstChild;if (a != filterField)return false;}return true;").toString();
