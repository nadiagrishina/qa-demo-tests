...
public class CouponsSortByStartDate23013Test {
    ...
    @Test
    public void test23013CouponsSortByStartDateASCDESC() throws Exception {
        ...
        driver.findElement(By.cssSelector(".tabulator-col:nth-child(3) .tabulator-arrow")).click();
        String resultSortedTable = js.executeScript("let tabl = document.getElementsByClassName('tabulator-cells'); let columnValues = [];let a = document.getElementsByClassName('tabulator-cell');for(let i = 2; i < a.length; i+= 6){columnValues.push(a[i].title);}for(let i = 0; i <columnValues.length - 1; i++){if(tabl[i] <= tabl[i + 1]){return false;}return true;}").toString();
        try {
