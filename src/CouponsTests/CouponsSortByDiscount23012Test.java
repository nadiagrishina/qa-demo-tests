...
public class CouponsSortByDiscount23012Test {
    ...
    @Test
    public void test23012CouponsSortByDiscountASCDESC() throws Exception {
        ...
        driver.findElement(By.cssSelector(".tabulator-col:nth-child(2) .tabulator-arrow")).click();
        String resultSortedTable = js.executeScript("let tabl = document.getElementsByClassName('tabulator-cells'); let columnValues = [];let a = document.getElementsByClassName('tabulator-cell');for(let i = 1; i < a.length; i+= 6){columnValues.push(a[i].title);}for(let i = 0; i <columnValues.length - 1; i++){if(tabl[i] <= tabl[i + 1]){return false;}return true;}").toString();
        try {
