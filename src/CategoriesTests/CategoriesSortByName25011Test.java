...
public class CategoriesSortByName25011Test {
    ...
    @Test
    public void test25011CategoriesASCDESC() throws Exception {
        ...
        WebElement element = driver.findElement(By.xpath("//div[@id='dataTable']/div/div/div/div/div/div[2]/div"));
        Actions actions = new Actions(driver);
        actions.moveToElement(element).click().build().perform();
        String resultSortedTable = js.executeScript("let tabl = document.getElementsByClassName('tabulator-cells');let columnValues = [];let a = document.getElementsByClassName('tabulator-cell');for(let i = 0; i < a.length; i+= 5){columnValues.push(a[i].title);}for(let i = 0; i <columnValues.length - 1; i++){if(tabl[i] <= tabl[i + 1]){return false;}return true;}").toString();
        try {
            assertEquals("true", resultSortedTable);
        } catch (Error e) {
            BufferHelper.appendBuffer(e.toString());
        }
        element = driver.findElement(By.xpath("//div[@id='dataTable']/div/div/div/div/div/div[2]/div"));
